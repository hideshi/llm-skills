...
> **📐 API仕様の補足（2026-09-19 調査）**:
> - **レスポンス形式の指定**: OpenAI の Structured Outputs のような任意JSONスキーマ指定はありません。`questions` の定義（キー名・プリミティブ種別・選択肢キー・レベル定義）がそのまま `answers` の構造になり、各回答のフィールドはプリミティブごとに固定です。
> - **state の形式**: 文字列だけでなくオブジェクト・配列も可（例: チケット本文・注文データ・ポリシー文を構造化して1つの state として投入）。
> - **制約**: choice は 2〜255 選択肢、score は 2〜10 レベル（クライアント側で検証）。各選択肢は数トークンを消費。コンテキスト上限は state＋全質問で 64k トークン、state＋最長の1質問で 32k トークン。
> - **noul の criteria**: 任意で `{"true": "...", "false": "..."}` を指定し、Yes/No 各側の定義を明示できます（③参照）。
> - ソース: apidog「How to Get a Jev API Key」/ Cloudflare AI docs / DEV Community「How to Use Jev」/ flaviocopes「A deep dive into Jev」（いずれも2026-09-19確認）
...
- **任意の `criteria`**: Yes/No の境界が微妙な場合、`{"true": "...", "false": "..."}` で各側の定義を明示できる（HTTP API 形式の例）:
  ```json
  {
    "type": "noul",
    "instructions": "Does this input violate our safety guidelines?",
...
- **criteria と矛盾させない**: instructions が聞いていることと criteria が説明していることがずれると回答精度が劣化する。criteria は instructions の延長として記述する。
...
- **脱出選択肢を用意する**: どれにも当てはまらない入力を無理やり分類させないため `none` / `other` を用意し、その業務上の扱い（レビュー送り等）を決めておく。公開ガイドでも `other` の付与が推奨されている（2026-09-19確認）。
...
- **絞り込みすぎず、無関係なものも足さない**: 業務上あり得る選択肢を短絡的に絞る（shortlist）と、どれも不正解な入力に「最も近い誤選択」を強制してしまう。公開ガイドはフルリスト投入を推奨しており、上限は255選択肢（各選択肢は数トークン消費）。一方、選択肢が増えるほど1選択肢あたりの確率は分散し confidence は下がるため、数十を超える大量分類は大分類→小分類の2段階（階層化）を検討する。
- **複合ラベルの主タイプ優先**: 1入力に複数タイプが同居しうる場合、主タイプ（primary）の優先規則を文書化し、設計チェックリストに含める（例: 安全上最重のタイプを優先、同時成立時のタイブレーク）。後段のラベル付け・較正が同じ規則を使えるようにする。

### score（レベル定義）の設計原則

- **各レベルに行動アンカー（観察可能な具体特徴）を書く**: `Medium` だけでなく `Needs response within 24 hours` のように、レベル間の区別が入力から判定可能な定義にする。
- **段階数は3〜5を目安**: 隣接レベルの区別が人間にも付かない段階数は、モデルでも較正が崩れやすい（API上の制約は2〜10レベル）。
- **順序尺度であることを意識**: 返り値の `score` は確率加重の期待値（1.85 等）であり、レベル間の「間隔」に業務的意味を持たせない。分岐に使う閾値は期待値ではなく `probabilities`（各レベルの確率分布）側で設計する（理由は後述）。
- **意味の一意性（違反強度 ≠ 運用温度）**: Score は「何の順序」を測るかを一文で固定する。違反の強さ（severity）と、温度／自動公開リスク（運用アクションの厳しさ）を同一スコアに混在させない。混在が必要なら question を分割する。
- **`none` でも medium/high があり得る**: 違反カテゴリが `none` でも、別軸の Score（例: 公開影響・緊急性）が medium/high になり得るなら、レベル定義とその分岐を明示する（「none なら常に low」と暗黙仮定しない）。
...
- **0.5 は「中間」ではない**: `noul = 0.5` は「どちらとも言えない（判断不能）」を意味し、「中程度のYes」ではない。度合いを測りたい場合は score を使い、Yes/No は明確に答えられる条件として定義する。

### state（判定対象入力）の設計原則

- **判定に必要な情報だけを入れる**: 無関係な長文はトークンコスト増とノイズになる。一方、判定に必要な文脈（直前のやり取り等）を欠くと精度が落ちる。必要十分かどうかは評価セットで確認する。
- **構造化データはそのまま渡せる**: state は文字列に加えてオブジェクト・配列も可（§1 の API仕様補足参照）。チケット＋注文データ＋ポリシー文のような複数情報源は、無理に平文へ結合せず構造を保って渡せる。いずれの形式でもフォーマットは固定する。
...
- **敵対的入力を想定する**: state はデータとして扱われるが、分類を誘導するよう加工されたテキスト（プロンプトインジェクション的な内容）で回答が動き得る。ユーザー制御の入力を state に含む場合は脅威モデルとして扱い、敵対的入力を評価セットに含めてテストする（セキュリティレビューと連携）。
- **長文は切り出す**: 判定対象が長文の場合、関連箇所の抽出やチャンク分割を前処理で行い、全文投入との精度比較を評価セットで行う。
- **追加フィールドの許可条件**: state にフィールドを足すときは、次をすべて満たすこと。(a) その判断に必要な事実である (b) ラベルや正解から導出した値ではない (c) 追加前／追加後を同一評価セットで比較できる。満たせない追加は見送り、比較評価の計画を HITL 確認にする。

### 複数 question の呼出（並列 / 逐次）

- **並列を既定**: 同一 state に対する複数 question は、依存が無い限り **1 リクエスト内の並列**（同一 `systemOne` / 同等呼出にまとめる）を既定とする。
- **依存の無い逐次は非推奨**: 前問の回答を後問の state に入れないのに逐次化するのは、レイテンシとコストが増えるだけで利点が薄い。**強い禁止（ハード禁止）にはしない**が、設計レビューでは理由の明示を求める。
- **逐次が妥当な場合**: 後問の instructions / state が前問の出力に本当に依存するときのみ。依存点と失敗時の Safe Default を設計ブロックに書く。
...
- **平坦な分布は設計側のシグナル**: 全選択肢にほぼ均等な確率（平坦な分布）が返る場合、モデルの混乱ではなく「criteria が state から区別不能」という設計側の問題であることが多い。閾値をいじる前に criteria の記述を見直す。
- **変更したら閾値を再較正**: 質問文を変えると確率分布が変わるため、既存の `blockThreshold` / `allowThreshold` は無効とみなし、検証ステータスを `PENDING` に戻した上で §5 のプロセスで再選定する。
