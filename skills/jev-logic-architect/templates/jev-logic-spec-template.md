...
- [ ] **監視・運用（観測契約に基づく実装）**:
  - [ ] 観測契約の設計 → **`jev-observability`**（判定イベント・時系列メトリクス・アラート意図・保持境界。指標の列挙はそちらで一元化）
  - [ ] `jev-observability` の契約を参照したログ／メトリクス／アラートの**実装** → SRE / MLOps
  - [ ] ドリフト疑い時の再較正トリガ運用 → SRE / MLOps（再較正実行は `jev-calibration`、スキーマ変更は本スキルへ差し戻し）


---

## 設計チェックリスト（content / 複合判定向け追記）

- [ ] **Score の意味が一意**（違反強度と温度／自動公開リスクを混同していない）。`none` でも medium/high があり得るなら明示した。
- [ ] **Choice 複合の主タイプ優先規則**を書き、タイブレークを定義した。
- [ ] **複数 question は並列既定**。依存の無い逐次呼出にする場合は理由を備考に残した（強い禁止ではない）。
- [ ] **state 追加フィールド**について (a) 判断に必要な事実 (b) ラベル由来でない (c) 追加前後の比較評価、を満たすか確認した。
