...
> **📌 本ドキュメントの数値はすべて参考値です。** 価格・レイテンシは調査日時点の一次情報または公称値であり、最新性・正確性は保証しません。**Jev の導入・利用に際しては、利用者が必ず事前に最新価格の確認と自環境での実測（PoC）を行ってください。**

## 1. 単価リファレンス (2026-09-18 調査・一次情報基準)

> **⚠️ 本表は調査日時点の参考値です。そのまま見積もりに使用しないでください。**
> 見積もりを行う際は、**必ず各ベンダーの公式価格ページ（typesafe.ai / openai.com/pricing / anthropic.com/pricing / ai.google.dev/pricing）から最新価格を取得し直し**、取得日と出典をレポートに記録してください。
