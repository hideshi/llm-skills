...
1b. **業務結果の重篤度（必須・投稿の深刻度と混同しない）**
   - 要件文書から、または HITL で、共有語彙 §8 の次を案件ごとに記録する（欠けたら推測せず確認事項にする）:
     1. 失敗モードごとの相対コスト（FN vs FP、不可逆の有無 等）
     2. リスク区分（高/中/低）＋理由一文
     3. ラボ／本番ゲートで**見る指標**と**見ない指標**（案件ごと）
   - **投稿の深刻度**（Score の low/medium/high）と同一フィールドにしない。全案件に同じ「見逃し優先」をスキル既定にしない。
   - 下流転記先: architect ADR、calibration ゲート／コスト行列、定形レポート §1（詳細は共有語彙 §8）。
