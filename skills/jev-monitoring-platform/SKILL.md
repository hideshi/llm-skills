...
2. **クラウド中立コアの要件化**
   ...
   - **取り込み**: 判定経路からのイベント／メトリクス投入。**人手最終判定イベント**（観測契約の延長）も取り込み対象に含める
   - **時系列ストア**: 集計・切り口クエリ（不一致率・理由コード切り口を含む場合は観測契約に従う）
...
3. **監視アプリ要件（最小）**
   - 取り込み確認（**人手最終判定イベントを含む**）、時系列ビュー、切り口、メタ／閾値版ドリルダウン、認証・認可
   - **含めない（後段）**: アラート配信の実装、ドリフト自動再較正の実行 UI（意図は観測契約側。実装は委譲）。政策スナップショット／再較正承認フローの詳細も本版外（読み取り共有面が必要である旨は試験参照に既出）
