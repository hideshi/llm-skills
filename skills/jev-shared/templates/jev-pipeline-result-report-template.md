...
> **用途**: 候補検出 → 論理設計 → 評価セット整備 → 較正 → shadow 評価 → 監視設計 → 監視基盤要件、の一連工程が**区切り**に達したときに、毎回同じ見出しで出す利用者向け要約。
> **タイトル**: 利用者向け本文の H1 は上記のとおり。**『（定形）』を付けない**。定形はスキル／共有語彙側の種別名（「定形結果レポート」）であり、チャット貼付の見出しには使わない。§1〜§3 も `対象` / `プロセス` / `結果` とし、括弧書きの補足は見出しに載せない（注記・用途ブロックへ）。
> **配信**: 本テンプレを埋めた**本文を、区切り時に利用者チャットへ全文貼る**。あわせて入力指定パスへファイル保存する。両方必須。添付のみ・要約のみ・ファイルのみは契約未達。「出しますか？」の確認は不要（自動義務）。詳細較正表はチャットに出さずパス参照。
...
| 業務結果の重篤度（要約） | 失敗モード相対コスト・コスト非対称の一文。**投稿の深刻度（Score）と別**（共有語彙 §8） |
| ゲート指標方針 | ラボ／本番で**見る指標**／**見ない指標**（入力転記） |
| Safe Default（要約） | |
...
| 7 | 監視基盤要件 | `jev-monitoring-platform` | `platformStatus:` `drafted` → `reviewed`（任意 HITL） | | 既定 CF・他クラウド写像。実装はしない |
...
- **ゲート／方針との関係**: [どの失敗モード・コスト非対称を優先して直したか。要件の業務結果の重篤度（共有語彙 §8）から転記]
...
| 条件 | 次 |
| :--- | :--- |
| `validationStatus: VALIDATED` かつ `shadowStatus: shadowed` | **監視設計**（`jev-observability`）→ **監視基盤要件**（`jev-monitoring-platform`）のうえ、実装／**リリース管理**へ委譲（canary / 全量 / ロールバックは本鎖の外） |
