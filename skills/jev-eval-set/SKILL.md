...
2. **サンプリング & 分割方針の文書化**
   ...
   - **層化軸（推奨セット）**: 短文 / 長文 / 皮肉・嫌味 / 複合 / 境界、および **ロケール（JA/EN）**。案件で不要な軸は省略理由を書く。
...
5. **ラベル成果物と enrichment の分離**
   - **freeze 対象**はラベル付き評価用 JSONL（およびスキーマ／分割定義）とする。
   - レポート理由文、親投稿、スレッド文脈、デバッグ用メタなどの **enrichment** はラベル JSONL と分離して保管し、freeze ハッシュの対象外とする（混入すると版が不安定になる）。enrichment を評価に使う場合は別契約・別版として明示する。
...
7. **HITL トリガの列挙（該当をレポートに残す）**
   - criteria / ラベル定義の **意味** 変更（文言の言い換えのみは除く）
   - 境界ケースの系統的不一致（層別に偏る誤り）
   - 複合ラベルの主タイプ優先規則が未定義、またはアノテータ間で食い違う
   - ロケール（JA/EN）間で指標が大きく乖離する
   - enrichment をラベルと誤って同一 JSONL に混在させていた
   - `datasetVersion` 更新に伴い較正側を `PENDING` に戻す必要が生じた
