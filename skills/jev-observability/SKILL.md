...
- **判定イベント契約**（1 判定あたり何を残すか。**人手最終判定イベント**を含む）
- **時系列メトリクス契約**（成功率・レイテンシ・REVIEW 率・不一致率／理由コード切り口等）
- **アラート意図**（何が異常か。閾値数値は入力）
- **保持・プライバシー境界**（生本文は既定最小化＋境界拡張は審査。ケース指紋は最小識別子）
...
2. **判定イベント契約の定義**
   ...
   - **人手最終判定がある経路**では、対になる Jev 回答（ラベル／Score／noul／経路＋問い版／閾値版／ポリシー版）、人手最終判断（同一ラベル空間）、不一致（agree／diff＋ずれ問い）、理由コード（短コード必須）、ケース指紋、版リンクを**強く推奨**（当該経路では必須）として契約に含める。
   - 生本文・PII はデフォルトでイベントに入れず、入れる場合は審査要否を記録する（**既定最小化＋境界拡張は審査**）。ケース指紋は ID・職種ファミリー等の最小識別に留め、生本文は原則保持しない／マスク／短保持。

3. **時系列メトリクス契約の定義**
   - 最低限: 成功率（または完了率）、レイテンシ p50/p95、REVIEW 率、フォールバック率、Safe Default 率。
   - 人手最終判定を契約する場合の追加切り口: 問いごとの不一致率、FN 寄り／FP 寄り、理由コード別。
   - 切り口例（案件に応じて入力から選ぶ）: Requirement ID、担当部署、緊急度、人手レビュー要否、経路、`validationStatus`、問い版・理由コード。
   - 合格ライン・アラート閾値の**数値は入力**。スキルは「どの指標を定義するか」だけを固定する。
   - 不一致＋理由コードから較正側トリアージ（`NEEDS_MORE_DATA`／問い改訂／閾値掃引）へ渡す前提を一文で書き、詳細は `jev-calibration` に委譲する。**誰がいつ再較正を承認するかは本スキルで固定しない。**
...
| 項目 | 内容 |
| :--- | :--- |
| 判定イベント契約 | 必須／任意フィールド、禁止フィールド。**人手最終判定イベント**（対になる Jev 回答・人手最終・不一致・理由コード・ケース指紋・版リンク）を含む |
| 時系列メトリクス契約 | 指標名・定義・次元・成果物参照（数値閾値は入力）。不一致率／理由コード切り口を含む場合はその旨 |
